...
class EfficientMarketDataProcessor:
    ...
    def create_enhanced_labels(self, features_df: pd.DataFrame, price_data, lookforward_days=1):
        """
        增强版标签生成（替换原函数）
        """
        print("创建增强版智能标签...")
...
class RealisticBacktester:
    ...
    def calculate_position_size(self, current_capital: float, confidence: float,
        ...
        if confidence > 0.6 and volatility < 0.05:  # 高信心低波动
            kelly_fraction = min(0.3, confidence - 0.5)
        ...
        # 波动率调整
        vol_adjustment = max(0.6, 1 - volatility * 10)
    ...
    def run_backtest(self, predictions: List[int], confidence_scores: List[float],
        ...
        # ==================== 分阶段风险控制参数 ====================
        # 阶段1: 初始止损止盈（刚入场时严格保护）
        initial_stop_loss = 0.003  # 初始止损0.3%
        initial_take_profit = 0.01  # 初始止盈1%

        # 阶段2: 移动止损（有盈利后启动跟踪保护）
        trailing_stop_activation = 0.006  # 盈利0.6%后启动移动止损
        trailing_stop_distance = 0.004  # 移动止损距离0.4%（从最高点回落0.4%触发）

        # 阶段3: 动态止盈（根据盈利幅度调整止盈点）
        dynamic_take_profit_levels = {
            0.005: 0.012,  # 盈利0.5%时，止盈设为1.2%
            0.01: 0.04,  # 盈利1%时，止盈设为4%
            0.03: 0.1,  # 盈利3%时，止盈设为10%
            0.09: 0.2  # 盈利9%时，止盈设为20%
        }
        ...
        if position > 0:
            ...
            close_trade = self.execute_trade(0, final_price, capital, 0.5, volatilities[-1], timestamps[-1])
...
class ImprovedTransformerPredictor(nn.Module):
    ...
    def __init__(self, input_dim: int, num_heads: int = 4, num_layers: int = 2,
                 dropout: float = 0.2, dim_feedforward: int = 128,
                 pooling: str = 'last'):
